    ...
    def myAtoi(self, str):
        ...
        for char in str:
            if not char.isnumeric():
                ...
                # characters that would not cause an escape
                if char not in [' ', '+', '-']:
                    return int(returnInt)
                if char == '-' and len(returnInt) == 0:
                    # Checks length too to ensure that a minus is only at the front
                    returnInt = '-'
                elif char == '-' and len(returnInt) != 0:
                    return int(returnInt)
            else:
                returnInt += char
        return int(returnInt)
